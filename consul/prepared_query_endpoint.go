...
func (p *PreparedQuery) Apply(args *structs.PreparedQueryRequest, reply *string) (err error) {
	...
	*reply = args.Query.ID
	...
	// Extract the prefix from query being created, or the existing query
	// that's being modified.
	var prefix string
	if args.Op == structs.PreparedQueryCreate {
		prefix = args.Query.GetACLPrefix()
	} else {
		state := p.srv.fsm.State()
		_, query, err := state.PreparedQueryGet(args.Query.ID)
		if err != nil {
	...
	// Make sure the token has write access to this prefix.
	acl, err := p.srv.resolveToken(args.Token)
	if err != nil {
		return err
	}
	if acl != nil && !acl.PreparedQueryWrite(prefix) {
		p.srv.logger.Printf("[DEBUG] consul.prepared_query: Operation on prepared query '%s' denied due to ACLs", args.Query.ID)
